 ...
 * Description: Mimics Revolve PDP with portrait gallery, centered buy box, and clean tabs (Description | Location | Who's Attending | Reviews)
 * Version: 4.0.0
 * Author: PTP
...
add_action('woocommerce_after_single_product_summary', function(){
  ...
  // ATTRS
  $date    = trim($p->get_attribute('pa_date_range'));
  $time    = trim($p->get_attribute('pa_time_range'));
  $ages    = trim($p->get_attribute('pa_ages'));
  $venue   = trim($p->get_attribute('pa_venue_name'));
  $address = trim($p->get_attribute('pa_venue_address'));

  // Copy for Description tab
  ...
  if(empty($att)) $att = ['NCAA mentors announced soon'];
  ...
  // Tabs (Description | Location | Attending | Reviews)
  ...
  if ($map_src){
    ...
    echo '<div class="ptp-map-embed">'
       . '<button type="button" class="map-loader" data-src="'.esc_url($map_src).'">Tap to load map</button>'
...
// ---------- Related products: 4-up wide, portrait images ----------
...
// ---------- Behavior (tabs, map lazy, Trustindex lazy) ----------
add_action('wp_footer', function(){ ?>
<script>
(function(){
  var root=document.getElementById('ptp-revolve-v4'); if(!root) return;
  // Tabs
  ...
  function openTab(id){
    ...
    var t=[].find.call(tabs, function(el){return el.getAttribute('aria-controls')===id});
    var pane=root.querySelector('#'+id);
    if(t){ t.classList.add('is-active'); t.setAttribute('aria-selected','true'); }
    if(pane){ pane.classList.add('is-active'); }
    if (window.matchMedia('(max-width:1099px)').matches){ root.scrollIntoView({behavior:'smooth', block:'start'}); }
  }
  tabs.forEach(function(t){ t.addEventListener('click', function(){ openTab(t.getAttribute('aria-controls')); }); });
  ...
  var tLoc=[].find.call(tabs, function(el){return el.getAttribute('aria-controls')==='tab-loc'}); 
  if(tLoc){ tLoc.addEventListener('click', loadMap); }
  ...
  function loadTI(){ 
    ...
    if(!ph) return; 
    ph.outerHTML = <?php echo json_encode(do_shortcode('[trustindex no-registration=google]')); ?>; 
    tiLoaded=true; 
  }
  var tRev=[].find.call(tabs, function(el){return el.getAttribute('aria-controls')==='tab-rev'}); 
  if(tRev){ tRev.addEventListener('click', loadTI); }
})();
</script>
<?php }, 10);
