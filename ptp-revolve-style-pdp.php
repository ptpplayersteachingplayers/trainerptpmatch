<?php
/**
 * PTP — Revolve-Style PDP v4 (Targeted, Spacing-Safe)
...
#ptp-revolve-v4 .att{list-style:none;padding:0;margin:0;text-align:center}
#ptp-revolve-v4 .att li{padding:8px 0;border-bottom:1px solid #f3f4f6}
#ptp-revolve-v4 .att li:last-child{border-bottom:none}

/* --- Accessibility improvements --- */
#ptp-revolve-v4 .tab:focus{outline:2px solid #FCB900;outline-offset:2px}
#ptp-revolve-v4 .map-loader:focus{outline:2px solid #FCB900;outline-offset:2px}

/* --- Responsive improvements --- */
@media (max-width:768px){
  #ptp-revolve-v4 .pr-tabs{flex-wrap:wrap;gap:8px}
  #ptp-revolve-v4 .tab{padding:8px 12px;font-size:0.9rem}
  #ptp-revolve-v4 .chip{padding:6px 10px;font-size:0.9rem}
}
</style>
<?php });
...
(function(){
  ...
  function openTab(id){
    ...
    if(t){ t.classList.add('is-active'); t.setAttribute('aria-selected','true'); if(bar){ bar.textContent=t.textContent; } }
  ...
  function initMapLazy(){
    var wrap=root.querySelector('#tab-loc .ptp-map-embed'); if(!wrap) return;
    var btn=wrap.querySelector('.map-loader'); var iframe=wrap.querySelector('iframe'); var loaded=false;
    function doLoad(){ if(loaded||!btn||!iframe) return; var src=btn.getAttribute('data-src'); if(!src) return;
      iframe.src=src; iframe.style.display='block'; btn.remove(); loaded=true;
    }
    if(btn) btn.addEventListener('click', doLoad);
    if (root.querySelector('#tab-loc.panel.is-active')) doLoad();
  }
  var tLoc=[].find.call(tabs,function(el){return el.getAttribute('aria-controls')==='tab-loc'}); if(tLoc){ tLoc.addEventListener('click', initMapLazy); }
